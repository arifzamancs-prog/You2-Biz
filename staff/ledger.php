...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if(($_POST['ajax'] ?? '') === '1' && in_array($form_action, ['create_payment_type', 'update_payment_type', 'delete_payment_type'], true)){
        header('Content-Type: application/json; charset=utf-8');
        $reply = static function($success, $message, $data = []){
        ...
        if($form_action === 'delete_payment_type'){
            ...
            if(!$type){ $reply(false, 'Payment type not found.'); }
            if(staff_ledger_payment_type_used($conn, $user_id, $type['type_key'])){ $reply(false, 'This payment type has ledger history and cannot be deleted.'); }
            ...
            $reply(mysqli_stmt_execute($delete_stmt), 'Payment type deleted successfully.', ['type_id' => $type_id, 'type_key' => $type['type_key']]);
        }

        $type_name = trim((string)($_POST['type_name'] ?? ''));
        $type_key = staff_ledger_payment_type_key($type_name);
        if($type_name === '' || $type_key === ''){ $reply(false, 'Payment type name is required.'); }
        if($type_key === 'salary'){ $reply(false, 'Salary is a reserved payment type.'); }

        if($form_action === 'update_payment_type'){
            ...
            if(!$old_type){ $reply(false, 'Payment type not found.'); }
            ...
            try { $updated = mysqli_stmt_execute($update_stmt); } catch(Throwable $exception) { $updated = false; }
            if(!$updated){ $reply(false, 'Payment type could not be updated. This name may already exist.'); }
            ...
            $reply(true, 'Payment type updated successfully.', ['type' => ['id' => $type_id, 'type_key' => $type_key, 'type_name' => $type_name], 'old_type_key' => $old_type['type_key']]);
        }
        ...
        try { $created = mysqli_stmt_execute($insert_stmt); } catch(Throwable $exception) { $created = false; }
        if(!$created){ $reply(false, 'Payment type could not be created. This name may already exist.'); }
        $reply(true, 'Payment type created successfully.', ['type' => ['id' => (int)mysqli_insert_id($conn), 'type_key' => $type_key, 'type_name' => $type_name]]);
    }
...
<script>
$(function(){
  $("#entry_date_picker").datetimepicker({format:"DD-MM-YYYY", icons:{time:"far fa-clock", date:"far fa-calendar", up:"fas fa-arrow-up", down:"fas fa-arrow-down", previous:"fas fa-chevron-left", next:"fas fa-chevron-right", today:"far fa-calendar-check", clear:"far fa-trash-alt", close:"far fa-times-circle"}});
  var optionsBody=$("#payment-type-options-body");
  var feedback=$("#payment-type-feedback"); if(!feedback.length){ feedback=$("<div id=\"payment-type-feedback\" class=\"small mb-2\"></div>"); optionsBody.prepend(feedback); } optionsBody.find("form.d-inline").removeAttr("onsubmit");
  function paymentFeedback(message, success){ feedback.text(message).removeClass("text-success text-danger").addClass(success?"text-success":"text-danger"); }
  function esc(value){ return $("<div>").text(value || "").html(); }
  function requestType(data){ data.append("ajax","1"); return fetch("ledger.php",{method:"POST",body:data}).then(function(response){ return response.json().then(function(result){ if(!response.ok || !result.success){ throw new Error(result.message || "Request could not be completed."); } return result; }); }); }
  function addPaymentOption(type){ if($("select[name=entry_type] option[value=\""+type.type_key+"\"]").length===0){ $("select[name=entry_type]").append($("<option>",{value:type.type_key,text:type.type_name})); } }
  function paymentRow(type){ var id=type.id, formId="payment_type_update_"+id; return "<tr class=\"payment-type-row\"><td><div class=\"payment-type-view-row\"><div class=\"payment-type-name\">"+esc(type.type_name)+"</div><div class=\"payment-type-key\">"+esc(type.type_key)+"</div></div><form id=\""+formId+"\" method=\"post\" class=\"payment-type-edit-row mb-0\"><input type=\"hidden\" name=\"form_action\" value=\"update_payment_type\"><input type=\"hidden\" name=\"type_id\" value=\""+id+"\"><input type=\"text\" name=\"type_name\" class=\"form-control form-control-sm\" value=\""+esc(type.type_name)+"\" required></form></td><td><div class=\"payment-type-view-row\"><button type=\"button\" class=\"btn btn-info btn-sm payment-type-edit-btn\" title=\"Edit\"><i class=\"fas fa-edit\"></i></button> <form method=\"post\" class=\"d-inline payment-type-delete-form\"><input type=\"hidden\" name=\"form_action\" value=\"delete_payment_type\"><input type=\"hidden\" name=\"type_id\" value=\""+id+"\"><button class=\"btn btn-danger btn-sm\" title=\"Delete\"><i class=\"fas fa-trash\"></i></button></form></div><div class=\"payment-type-edit-row\"><button form=\""+formId+"\" class=\"btn btn-success btn-sm\" title=\"Save\"><i class=\"fas fa-save\"></i></button> <button type=\"button\" class=\"btn btn-secondary btn-sm payment-type-cancel-btn\" title=\"Cancel\"><i class=\"fas fa-times\"></i></button></div></td></tr>"; }
  optionsBody.on("click", ".payment-type-edit-btn", function(){ var row=$(this).closest(".payment-type-row"); $(".payment-type-row").not(row).removeClass("is-editing"); row.addClass("is-editing"); row.find("input[name=type_name]").trigger("focus").select(); });
  optionsBody.on("click", ".payment-type-cancel-btn", function(){ $(this).closest(".payment-type-row").removeClass("is-editing"); });
  optionsBody.on("submit", "form", function(event){ event.preventDefault(); var form=$(this), data=new FormData(this), action=data.get("form_action"); if(!action || action==="create_ledger") return; if(action==="delete_payment_type" && !confirm("Delete this payment type?")) return; var button=form.find("button:submit"); button.prop("disabled",true); requestType(data).then(function(result){ if(action==="create_payment_type"){ $(".payment-type-table tbody").append(paymentRow(result.type)); addPaymentOption(result.type); form.trigger("reset"); } else if(action==="update_payment_type"){ var row=form.closest(".payment-type-row"); row.find(".payment-type-name").text(result.type.type_name); row.find(".payment-type-key").text(result.type.type_key); row.removeClass("is-editing"); $("select[name=entry_type] option[value=\""+result.old_type_key+"\"]").attr("value",result.type.type_key).text(result.type.type_name); } else if(action==="delete_payment_type"){ form.closest(".payment-type-row").remove(); $("select[name=entry_type] option[value=\""+result.type_key+"\"]").remove(); } paymentFeedback(result.message,true); }).catch(function(error){ paymentFeedback(error.message,false); }).finally(function(){ button.prop("disabled",false); }); });
  $("#payment-type-visibility").on("click",function(){ var body=$("#payment-type-options-body"), open=body.hasClass("d-none"); body.toggleClass("d-none",!open); $(this).attr("aria-expanded",open?"true":"false").attr("title",open?"Hide payment type options":"Show payment type options").find("i").toggleClass("fa-eye",!open).toggleClass("fa-eye-slash",open); });
});
</script>
HTML;
