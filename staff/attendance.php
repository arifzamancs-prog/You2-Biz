...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($action === 'save_settings'){
        ...
        $absent_after = $_POST['absent_after_time'] ?? '18:00';
        $late_days = max(1, (int)($_POST['late_days_for_salary_cut'] ?? 3));
        // Every completed late-day threshold creates one salary-cut day.
        // The cut can be a fixed BDT amount or a percentage of assigned salary.
        $cut_type = ($_POST['salary_cut_type'] ?? 'fixed') === 'percentage' ? 'percentage' : 'fixed';
        $cut_value = max(0, (float)($_POST['salary_cut_value'] ?? 0));
        if($cut_type === 'percentage'){
            $cut_value = min(100, $cut_value);
        }
...
$salary_cut_display_value = (float)$settings['salary_cut_value'];
if($settings['salary_cut_type'] === 'percentage' && $salary_cut_display_value <= 0){
    $salary_cut_display_value = 3.33;
}elseif($settings['salary_cut_type'] !== 'percentage' && ($salary_cut_display_value <= 0 || abs($salary_cut_display_value - 100) < 0.001)){
    // Replace the old initial setting with the new fixed-cut default.
    $salary_cut_display_value = 666.66;
}
...
<div class="row">
 <div class="col-lg-7"><div class="card"><div class="card-header"><h3 class="card-title">Attendance Rules</h3></div><form id="attendance-rules-form" method="post"><div class="card-body row">
  ...
  <div class="col-md-6 form-group"><label>Absent After</label><input type="time" class="form-control" name="absent_after_time" value="<?= htmlspecialchars(substr($settings['absent_after_time'] ?? '18:00:00',0,5)) ?>"><small class="text-muted">Desktop login after this time will be marked Absent.</small></div>
  ...
  <div class="col-md-6 form-group"><label>Salary Cut Type</label><select class="form-control" name="salary_cut_type" id="salary-cut-type"><option value="fixed" <?= $settings['salary_cut_type'] === 'percentage' ? '' : 'selected' ?>>Cut Value Per Day (BDT)</option><option value="percentage" <?= $settings['salary_cut_type'] === 'percentage' ? 'selected' : '' ?>>Percentage of Salary</option></select><small class="text-muted">Percentage is calculated from the salary assigned in Staff Manage.</small></div>
  <div class="col-md-6 form-group"><label id="salary-cut-value-label"><?= $settings['salary_cut_type'] === 'percentage' ? 'Percentage of Salary' : 'Cut Value Per Day (BDT)' ?></label><input type="number" min="0" step="0.01" class="form-control" name="salary_cut_value" id="salary-cut-value" value="<?= number_format($salary_cut_display_value, 2, '.', '') ?>"><small id="salary-cut-value-help" class="text-muted"><?= $settings['salary_cut_type'] === 'percentage' ? 'This percentage of the assigned salary will be deducted for each salary-cut day.' : 'This amount will be deducted for each salary-cut day.' ?></small></div>
...
const updateSalaryCutValueLabel = () => {
 const isPercentage = document.getElementById('salary-cut-type').value === 'percentage';
 document.getElementById('salary-cut-value-label').textContent = isPercentage ? 'Percentage of Salary' : 'Cut Value Per Day (BDT)';
 document.getElementById('salary-cut-value-help').textContent = isPercentage
  ? 'This percentage of the assigned salary will be deducted for each salary-cut day.'
  : 'This amount will be deducted for each salary-cut day.';
 const input = document.getElementById('salary-cut-value');
 input.max = isPercentage ? '100' : '';
 input.value = isPercentage ? '3.33' : '666.66';
};
document.getElementById('salary-cut-type').addEventListener('change', updateSalaryCutValueLabel);
