...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($action === 'update_attendance_status'){
        ...
        if(!manager_can_modify()){
            $message = 'You do not have permission to update attendance status.';
            $message_type = 'danger';
        }else{
            if($attendance_id <= 0 || !in_array($status, $allowed_statuses, true)){
...
if(manager_can_modify() && $edit_attendance_id > 0){
...
$attendance_stmt = mysqli_prepare($conn, "SELECT a.*, s.name AS staff_name, s.staff_code, s.designation, COALESCE(NULLIF(b.branch_name,''), 'Head Office') AS branch_name
...
$log_years = mysqli_query($conn, "SELECT DISTINCT YEAR(attendance_date) AS attendance_year FROM staff_attendance_logs WHERE user_id={$user_id} UNION SELECT " . (int)date('Y') . " ORDER BY attendance_year DESC");
...
$today_roster_stmt = mysqli_prepare($conn, "SELECT s.id, s.name, s.staff_code, s.designation, COALESCE(NULLIF(b.branch_name,''), 'Head Office') AS branch_name, a.id AS attendance_id, a.login_at, a.login_ip, a.attendance_status, a.is_auto_absent
    FROM staff s
    LEFT JOIN branches b ON b.id=s.branch_id AND b.user_id=s.user_id
...
$yearly_attendance_summary = mysqli_query($conn, "SELECT s.id, s.name, s.staff_code, s.designation, COALESCE(NULLIF(b.branch_name,''), 'Head Office') AS branch_name,
    ...
    FROM staff s LEFT JOIN branches b ON b.id=s.branch_id AND b.user_id=s.user_id LEFT JOIN staff_attendance_logs a ON a.staff_id=s.id AND a.user_id=s.user_id AND a.attendance_date BETWEEN '" . mysqli_real_escape_string($conn, $attendance_filter_start) . "' AND '" . mysqli_real_escape_string($conn, $attendance_filter_end) . "'
    WHERE s.user_id={$user_id}{$attendance_staff_filter} GROUP BY s.id, s.name, s.staff_code, s.designation, b.branch_name ORDER BY s.name ASC");

require_once '../includes/header.php'; require_once '../includes/navbar.php'; require_once '../includes/sidebar.php';
?>
<?php if(isset($_GET['status_updated'])): ?><div class="alert alert-success">Attendance status updated.</div><script>window.history.replaceState({}, document.title, window.location.pathname + window.location.hash);</script><?php endif; ?>
<?php if(!manager_can_modify()): ?><style>.status-edit{display:none!important}</style><?php endif; ?>
<?php if($edit_attendance): ?><div id="attendance-status-editor" class="card"><div class="card-header"><h3 class="card-title">Update Attendance Status — <?=htmlspecialchars($edit_attendance['name'])?> <small class="text-muted">(<?=htmlspecialchars($edit_attendance['staff_code'])?>, <?=date('d-m-Y', strtotime($edit_attendance['attendance_date']))?>)</small></h3></div><form method="post"><div class="card-body row"><input type="hidden" name="action" value="update_attendance_status"><input type="hidden" name="attendance_id" value="<?= (int)$edit_attendance['id'] ?>"><div class="col-md-6 form-group"><label>Status</label><select name="attendance_status" class="form-control"><option value="present" <?=$edit_attendance['attendance_status']==='present'?'selected':''?>>Present</option><option value="late" <?=$edit_attendance['attendance_status']==='late'?'selected':''?>>Late</option><option value="absent" <?=$edit_attendance['attendance_status']==='absent'?'selected':''?>>Absent</option><option value="closed_day" <?=$edit_attendance['attendance_status']==='closed_day'?'selected':''?>>Office Closed Day</option><option value="casual_leave" <?=$edit_attendance['attendance_status']==='casual_leave'?'selected':''?>>Casual Leave</option><option value="medical_leave" <?=$edit_attendance['attendance_status']==='medical_leave'?'selected':''?>>Medical Leave</option></select></div><div class="col-md-6 form-group d-flex align-items-end"><button class="btn btn-primary mr-2"><i class="fas fa-save"></i> Update Status</button><a href="attendance.php" class="btn btn-secondary">Cancel</a></div></div></form></div><?php endif; ?>
...
<div class="card"><div class="card-header"><h3 class="card-title">Today's Attendance Roster</h3><div class="card-tools"><span class="text-muted"><?= date('d-m-Y') ?></span></div></div><div class="card-body table-responsive"><table class="table table-bordered table-sm mb-0"><thead><tr><th>Staff</th><th>Designation</th><th>Branch</th><th>Login Time</th><th>IP Address</th><th>Status</th></tr></thead><tbody><?php while($member=mysqli_fetch_assoc($today_roster)): $status=$member['attendance_status'] ?: ($today_is_closed ? 'closed_day' : (date('H:i:s') > ($settings['absent_after_time'] ?? '12:00:00') ? 'absent' : 'pending')); $auto_absent=!empty($member['is_auto_absent']) && $status==='absent'; $badge=$status==='present'?'success':($status==='late'?'warning':($status==='absent'?'danger':'secondary')); ?><tr><td><?= htmlspecialchars($member['name']) ?> <small class="text-muted">(<?= htmlspecialchars($member['staff_code']) ?>)</small></td><td><?= htmlspecialchars($member['designation']) ?></td><td><?= htmlspecialchars($member['branch_name']) ?></td><td><?= $auto_absent ? '—' : ($member['login_at'] ? date('h:i A', strtotime($member['login_at'])) : '—') ?></td><td><?= $auto_absent ? '—' : htmlspecialchars($member['login_ip'] ?: '—') ?></td><td><span class="badge badge-<?= $badge ?>"><?= htmlspecialchars(ucwords(str_replace('_',' ',$status))) ?></span></td></tr><?php endwhile; ?></tbody></table></div></div>
<div class="card"><div class="card-header"><h3 class="card-title">Staff Attendance Log</h3><div class="card-tools"><form method="get" class="form-inline"><input type="hidden" name="attendance_staff_id" value="<?= $attendance_staff_id ?>"><input type="hidden" name="attendance_summary_month" value="<?= $attendance_summary_month ?>"><input type="hidden" name="attendance_year" value="<?= $attendance_year ?>"><select name="month" class="form-control form-control-sm mr-2" aria-label="Log month"><?php for($log_month=1;$log_month<=12;$log_month++): ?><option value="<?= $log_month ?>" <?= $month===$log_month?'selected':'' ?>><?= date('F', mktime(0,0,0,$log_month,1)) ?></option><?php endfor; ?></select><select name="year" class="form-control form-control-sm mr-2" aria-label="Log year"><?php while($log_year_row=mysqli_fetch_assoc($log_years)): $log_year=(int)$log_year_row['attendance_year']; ?><option value="<?= $log_year ?>" <?= $year===$log_year?'selected':'' ?>><?= $log_year ?></option><?php endwhile; ?></select><button class="btn btn-outline-primary btn-sm mr-2" title="View selected month"><i class="fas fa-filter"></i> View</button><button type="submit" formaction="attendance_print.php" formtarget="_blank" class="btn btn-primary btn-sm"><i class="fas fa-print"></i> Print</button></form></div></div><div class="card-body table-responsive"><table id="example1" class="table table-bordered table-striped"><thead><tr><th>Date</th><th>Staff</th><th>Designation</th><th>Branch</th><th>Login Time</th><th>IP Address</th><th>Device</th><th>Status</th></tr></thead><tbody><?php while($row=mysqli_fetch_assoc($attendance)): $class=$row['attendance_status']==='present'?'success':($row['attendance_status']==='late'?'warning':($row['attendance_status']==='absent'?'danger':'secondary')); $auto_absent=!empty($row['is_auto_absent']) && $row['attendance_status']==='absent'; ?><tr><td><?= date('d-m-Y', strtotime($row['attendance_date'])) ?></td><td><?= htmlspecialchars($row['staff_name']) ?> <small class="text-muted">(<?= htmlspecialchars($row['staff_code']) ?>)</small></td><td><?= htmlspecialchars($row['designation']) ?></td><td><?= htmlspecialchars($row['branch_name']) ?></td><td><?= $auto_absent ? '—' : date('h:i A', strtotime($row['login_at'])) ?></td><td><?= $auto_absent ? '—' : htmlspecialchars($row['login_ip']) ?></td><td><?= $auto_absent ? 'Auto' : 'Desktop' ?></td><td><span class="badge badge-<?= $class ?> attendance-status-badge"><?= htmlspecialchars(ucwords(str_replace('_',' ',$row['attendance_status']))) ?></span><?php if(manager_can_modify()): ?> <a href="attendance.php?month=<?= $month ?>&year=<?= $year ?>&edit_attendance=<?= (int)$row['id'] ?>#attendance-status-editor" class="btn btn-outline-secondary btn-xs" title="Edit status"><i class="fas fa-edit"></i></a><?php endif; ?></td></tr><?php endwhile; ?></tbody></table></div></div>
...
<form method="get" class="row align-items-end mb-3"><input type="hidden" name="month" value="<?= $month ?>"><input type="hidden" name="year" value="<?= $year ?>"><div class="col-md-4 form-group mb-md-0"><label>Staff</label><select name="attendance_staff_id" class="form-control"><option value="0">All Staff</option><?php while($attendance_staff=mysqli_fetch_assoc($attendance_staffs)){ ?><option value="<?=$attendance_staff['id']?>" <?=$attendance_staff_id===(int)$attendance_staff['id']?'selected':''?>><?=htmlspecialchars($attendance_staff['name'])?> (<?=htmlspecialchars($attendance_staff['staff_code'])?>)</option><?php } ?></select></div><div class="col-md-3 form-group mb-md-0"><label>Month</label><select name="attendance_summary_month" class="form-control"><option value="0" <?=$attendance_summary_month===0?'selected':''?>>All Months</option><?php for($summary_month=1;$summary_month<=12;$summary_month++): ?><option value="<?=$summary_month?>" <?=$attendance_summary_month===$summary_month?'selected':''?>><?=date('F', mktime(0,0,0,$summary_month,1))?></option><?php endfor; ?></select></div><div class="col-md-2 form-group mb-md-0"><label>Year</label><select name="attendance_year" class="form-control"><?php while($year_row=mysqli_fetch_assoc($attendance_years)){ $year_value=(int)$year_row['attendance_year']; ?><option value="<?=$year_value?>" <?=$attendance_year===$year_value?'selected':''?>><?=$year_value?></option><?php } ?></select></div><div class="col-md-3 mt-3 mt-md-0"><button class="btn btn-primary mr-1"><i class="fas fa-filter"></i> View Attendance</button><button type="submit" formaction="attendance_summary_print.php" formtarget="_blank" class="btn btn-outline-primary"><i class="fas fa-print"></i> Print</button></div></form>
<div class="table-responsive"><table class="table table-bordered table-striped"><thead><tr><th>Staff</th><th>Designation</th><th>Branch</th><th class="text-center">Present</th><th class="text-center">Late</th><th class="text-center">Absent</th><th class="text-center">Casual Leave</th><th class="text-center">Medical Leave</th></tr></thead><tbody><?php while($yearly_summary=mysqli_fetch_assoc($yearly_attendance_summary)){ ?><tr><td><?=htmlspecialchars($yearly_summary['name'])?> <small class="text-muted">(<?=htmlspecialchars($yearly_summary['staff_code'])?>)</small></td><td><?=htmlspecialchars($yearly_summary['designation'] ?: '-')?></td><td><?= htmlspecialchars($yearly_summary['branch_name']) ?></td><td class="text-center"><span class="badge badge-success"><?=(int)$yearly_summary['present_count']?></span></td><td class="text-center"><span class="badge badge-warning"><?=(int)$yearly_summary['late_count']?></span></td><td class="text-center"><span class="badge badge-danger"><?=(int)$yearly_summary['absent_count']?></span></td><td class="text-center"><span class="badge badge-info"><?=(int)$yearly_summary['casual_leave_count']?></span></td><td class="text-center"><span class="badge badge-primary"><?=(int)$yearly_summary['medical_leave_count']?></span></td></tr><?php } ?></tbody></table></div>
</div></div>
<div class="modal fade" id="attendanceStatusModal" tabindex="-1"><div class="modal-dialog"><form id="attendance-status-form" class="modal-content"><div class="modal-header"><h5 class="modal-title">Update Attendance Status</h5><button type="button" class="close" data-dismiss="modal">&times;</button></div><div class="modal-body"><input type="hidden" name="action" value="update_attendance_status"><input type="hidden" name="ajax" value="1"><input type="hidden" name="attendance_id" id="attendance-status-id"><div class="form-group mb-0"><label>Status</label><select name="attendance_status" id="attendance-status-value" class="form-control"><option value="present">Present</option><option value="late">Late</option><option value="absent">Absent</option><option value="closed_day">Office Closed Day</option><option value="casual_leave">Casual Leave</option><option value="medical_leave">Medical Leave</option></select><small class="text-muted">Casual Leave and Medical Leave do not create a salary cut.</small></div></div><div class="modal-footer"><span id="status-feedback" class="text-danger mr-auto"></span><button type="submit" class="btn btn-primary">Update Status</button></div></form></div></div>
<script>
$(function() {
});
