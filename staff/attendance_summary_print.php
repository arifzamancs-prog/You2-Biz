...
body{margin:0;background:#eef2f7;font-family:Arial,sans-serif;color:#172033}.report{box-sizing:border-box;width:297mm;min-height:210mm;margin:18px auto;background:#fff;padding:16mm;box-shadow:0 2px 12px #0002}.heading{border-bottom:3px solid #1677e8;padding-bottom:14px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:flex-start}.heading h1{font-size:24px;margin:0 0 5px}.heading p{margin:0;color:#5a677a}.period{text-align:right}.period strong{display:block;font-size:18px;margin-bottom:5px}table{width:100%;border-collapse:collapse;font-size:12px}th{background:#1677e8;color:#fff;text-align:center;padding:9px 7px}th:first-child,th:nth-child(2){text-align:left}td{padding:8px;border-bottom:1px solid #dce3ec;text-align:center}td:first-child,td:nth-child(2){text-align:left}th:nth-child(1),td:nth-child(1){width:8%}th:nth-child(8),td:nth-child(8),th:nth-child(9),td:nth-child(9){width:13%}tr:nth-child(even) td{background:#f7f9fc}.print-button{position:fixed;right:22px;top:20px;border:0;border-radius:5px;background:#1677e8;color:#fff;padding:10px 16px;font-size:14px;cursor:pointer}@media print{body{background:#fff}.report{width:auto;min-height:0;margin:0;box-shadow:none;padding:0}.print-button{display:none}@page{size:A4 landscape;margin:12mm}}</style></head><body><button class="print-button" onclick="window.print()">Print Report</button><main class="report"><header class="heading"><div><h1>Yearly Attendance Summary</h1><p><?= $company_name ?></p></div><div class="period"><strong><?= htmlspecialchars($period) ?></strong><p>Generated: <?= date('d-m-Y h:i A') ?></p></div></header><table><thead><tr><th>ID</th><th>Name</th><th>Designation</th><th>Branch</th><th>Present</th><th>Late</th><th>Absent</th><th>Casual Leave</th><th>Medical Leave</th></tr></thead><tbody><?php if(mysqli_num_rows($summary) === 0): ?><tr><td colspan="9" style="text-align:center;color:#697586">No staff records found.</td></tr><?php else: while($row=mysqli_fetch_assoc($summary)): ?><tr><td><?= htmlspecialchars($row['staff_code']) ?></td><td><?= htmlspecialchars($row['name']) ?></td><td><?= htmlspecialchars($row['designation'] ?: '-') ?></td><td><?= htmlspecialchars($row['branch_name']) ?></td><td><?= (int)$row['present_count'] ?></td><td><?= (int)$row['late_count'] ?></td><td><?= (int)$row['absent_count'] ?></td><td><?= (int)$row['casual_leave_count'] ?></td><td><?= (int)$row['medical_leave_count'] ?></td></tr><?php endwhile; endif; ?></tbody></table></main></body></html>
