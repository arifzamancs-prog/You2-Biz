...
<?php $rename_types_result = mysqli_query($conn, "SELECT id, type_name FROM booking_invoice_types WHERE user_id={$user_id} AND status='active' AND type_key<>'profit_return' ORDER BY type_name"); ?>
<form method="post" class="mb-4"><input type="hidden" name="action" value="rename"><div class="row"><div class="col-md-4 form-group"><label>Payment Type</label><select name="type_id" class="form-control" required><option value="">Select Payment Type</option><?php while($rename_type = mysqli_fetch_assoc($rename_types_result)){ ?><option value="<?= (int)$rename_type['id']; ?>"><?= htmlspecialchars($rename_type['type_name']); ?></option><?php } ?></select></div><div class="col-md-5 form-group"><label>New Name</label><input type="text" name="type_name" class="form-control" maxlength="100" required></div><div class="col-md-2 form-group d-flex align-items-end"><button class="btn btn-info">Rename Type</button></div></div></form>
<table id="example1" class="table table-bordered table-striped"><thead><tr><th>Payment Type</th><th>Behavior</th><th>Status</th><th>Transactions</th><th width="100">Action</th></tr></thead><tbody><?php while($type = mysqli_fetch_assoc($types_result)){ $is_system_type = in_array($type['type_key'], $system_type_keys, true); ?><tr><td><?= htmlspecialchars($type['type_name']); ?></td><td><span class="badge badge-<?= $type['behavior'] === 'income' ? 'success' : 'warning'; ?>"><?= htmlspecialchars(ucfirst($type['behavior'])); ?></span></td><td><span class="badge badge-<?= $type['status'] === 'active' ? 'success' : 'secondary'; ?>"><?= htmlspecialchars(ucfirst($type['status'])); ?></span></td><td><?= (int)$type['transaction_count']; ?></td><td><?php if($is_system_type){ ?><span class="text-muted"><i class="fas fa-lock"></i> Fixed</span><?php }elseif($type['status'] === 'active' && (int)$type['transaction_count'] > 0){ ?><button class="btn btn-secondary btn-sm" disabled title="This type has transactions"><i class="fas fa-trash"></i></button><?php }elseif($type['status'] === 'active'){ ?><form method="post" onsubmit="return confirm('Remove this payment type?');"><input type="hidden" name="action" value="delete"><input type="hidden" name="type_id" value="<?= (int)$type['id']; ?>"><button class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash"></i></button></form><?php } ?></td></tr><?php } ?></tbody></table>
</div></div>
