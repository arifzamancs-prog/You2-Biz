...
if(!$project){ die($labels['project'] . ' not found.'); }
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    if($name === ''){
        ...
        $message = $labels['project'] . ' could not be updated.';
    }
...
?>
<div class="card"><div class="card-header"><h3 class="card-title">Edit <?= htmlspecialchars($labels['project']); ?></h3></div><div class="card-body">
<?php if($message){ ?><div class="alert alert-danger"><?= htmlspecialchars($message); ?></div><?php } ?>
<form method="post"><div class="form-group"><label><?= htmlspecialchars($labels['project_name']); ?></label><input class="form-control" name="project_name" value="<?= htmlspecialchars($project['project_name']); ?>" required></div><div class="form-group"><label>Description</label><textarea class="form-control" name="description" rows="3"><?= htmlspecialchars($project['description'] ?? ''); ?></textarea></div><div class="form-group"><label>Status</label><select class="form-control" name="status"><option value="active" <?= ($project['status'] ?? 'active') === 'active' ? 'selected' : ''; ?>>Active</option><option value="inactive" <?= ($project['status'] ?? '') === 'inactive' ? 'selected' : ''; ?>>Inactive</option></select></div><button class="btn btn-primary"><?= htmlspecialchars($labels['project_update']); ?></button> <a href="projects.php" class="btn btn-secondary">Back</a></form>
</div></div>
